...
function [err_pos, err_rot]  = ur5TJcontrol(gdesired, ur5)

% Variables 
vk_thold = 1e-2;            % norm of twist components vk [m] 
wk_thold = deg2rad(0.1);    % norm of twist components wk [rad]
mu_thold = 1e-4;            % manipulability measure threshold
                            % to determine if the system is close to a singularity

% adaptive gain variables
Gain_tune_param = 3e-5;
R_per_sec = 2*pi*100;   % radians per second [rad/s], natural frequency ~ 100HZ
DR = 1;                 % damping ratio [0.75,1]
C = 0.02*3.6;           % C = K*V (0.02 < K < 0.03, 2 < V < 3.6 
                        % V: peak velocity of acurators

% adaptive Tstep variables
Tstep_fix_idx = 12;
OS_thold = 0.03;       % [m]
...
while (norm(vk) > vk_thold || norm(wk) > wk_thold)
    ...
    K = (R_per_sec*DR) / (C*Tstep) * Gain_tune_param;
    if idx == Tstep_fix_idx
        Tstep_traj = Tstep;
    end
    if idx >= Tstep_fix_idx && norm(vk) > OS_thold
        Tstep = Tstep_traj;
    end
...
%     fprintf('EE Z = %0.4f \n', gk(3,4));
    ...
    if idx < Tstep_fix_idx
        ...
        dt = max(0.01, Tstep);  % min dt value for smooth run
    end
    notpass = true;
    while notpass == true
